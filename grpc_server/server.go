...
func (s *Server) AddVal(ctx context.Context, keyVal *keyvalue.KeyValue) (*keyvalue.KeyValue, error) {
	if keyVal == nil || keyVal.Key == "" {
		log.Printf("GRPC_SERVER: AddVal received invalid keyval from client")
		err := status.Error(codes.InvalidArgument, "missing key.Val")
		return nil, err
	}

	log.Printf("GRPC_SERVER: Received AddVal message from client: key:%s, val:%s", keyVal.Key, keyVal.Val)

	existingVal, found := s.stored_vals[keyVal.Key]
	if found {
		log.Printf("GRPC_SERVER: AddVal attempted to add val with existing key entry. key:%s, val:%s", keyVal.Key, existingVal)
...
func (s *Server) GetVal(ctx context.Context, key *keyvalue.Key) (*keyvalue.KeyValue, error) {
	log.Printf("GRPC_SERVER: Received GetVal message from client: %s", key.Key)

	found_val, found := s.stored_vals[key.Key]
	if !found {
		log.Printf("GRPC_SERVER: Failed to locate kvp with key=%s", key.Key)
...
func (s *Server) GetAll(ctx context.Context, req *keyvalue.GetAllRequest) (*keyvalue.GetAllResponse, error) {
	log.Printf("GRPC_SERVER: Received GetAll message from client")
...
func (s *Server) DeleteVal(ctx context.Context, key *keyvalue.Key) (*keyvalue.KeyValue, error) {
	log.Printf("GRPC_SERVER: Received DeleteVal message from client: %s", key.Key)
...
func (s *Server) ModifyVal(ctx context.Context, keyval *keyvalue.KeyValue) (*keyvalue.KeyValue, error) {
	log.Printf("GRPC_SERVER: Received ModifyVal message from client: %s", keyval.Key)

	_, found := s.stored_vals[keyval.Key]
	if !found {
		log.Printf("GRPC_SERVER: Failed to locate kvp with key=%s", keyval.Key)
...
func main() {
	...
	if err != nil {
		log.Fatalf("GRPC_SERVER: Failed to listen on port 9000: %v", err)
	}
